...
pplot=function (model, xvar, by = NULL, breaks = NULL, partial = FALSE,
          leg.name = NULL, leg.labs = NULL, cols = c("red", "blue",
                                                     "green", "purple"), linesize = 1.5, transp = 0.5, rsize = 2,
          rcolor = "red", rcols = FALSE, rshapes = TRUE)
{
   require(visreg)
   require(ggplot2)
  ...
  if (!is.null(p$meta$by)) {
    ...
    plt= ggplot(p$fit, aes(x=p$fit[,1],y=p$fit$visregFit))+
      ...
      geom_errorbar(aes(ymin = p$fit$visregLwr, ymax = p$fit$visregUpr),width=.15,color="blue")
    plt=plt+xlab("")
