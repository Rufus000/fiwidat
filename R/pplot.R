...
pplot=function(model,xvar,by=NULL,breaks=NULL, partial=FALSE,leg.name=NULL,leg.labs=NULL,cols=c("red","blue","green","purple"), linesize=1.5,transp=0.5, rsize=2,rcolor="red",rcols=FALSE, rshapes=TRUE){
  ...
  if (!is.null(p$meta$by)){
    plt= ggplot(p$fit, aes(x=p$fit[,ix], y=visregFit,fill=as.factor(p$fit[,ib])))+
      ...
      geom_line(size=linesize)+xlab(p$meta$x)+ylab(p$meta$y)
    ...
    if (partial==T){
      plt=plt+geom_point(data=p$res,aes(x=p$res[,ix],y=visregRes),colour=rcolor,
